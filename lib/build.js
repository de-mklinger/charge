...
const buildGlobIgnore = (source, ignore, defaultMdxLayout) => {
  const fullIgnore = []

  if (defaultMdxLayout) {
    fullIgnore.push(`${source}/**/${defaultMdxLayout}`)
  }
  ...
  fullIgnore.push(...ignore.map((i) => `${source}/${i}`))
...
export default async ({
  ...
  outputMode,
  homepage = "",
  defaultMdxLayout = "default-mdx-layout.jsx",
  environment = "production",
}) => {
  let files = globSyncNormalize(`${source}/**/*`, {
    nodir: true,
    ignore: buildGlobIgnore(source, ignore, defaultMdxLayout),
  })
  ...
  let sourceFilesThatCouldBeBuilt = dependentsThatAreNotDependencies(sourceFiles).filter(
    (file) => pathBasename(file.path) !== "default-mdx-layout.html.jsx",
  )
