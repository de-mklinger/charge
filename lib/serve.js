...
import { resolve as pathResolve, join as pathJoin, basename as pathBasename } from "path"
import { existsSync, readFileSync } from "fs"
import BrowserSync from "browser-sync"
import logger from "./logger"
import build from "./build"
import { buildGlobIgnore, defaultMdxLayout } from "./utilities"
...
const fileIsDefaultLayout = (path) => {
  return pathBasename(path) === defaultMdxLayout
}

export default async ({ source, openBrowser = true, ignore, outputMode, port = 2468 }) => {
  ...
  browserSync.watch(
    ...
    async (event, file) => {
      if (fileIsAddedOrRemoved(event) || fileIsData(file) || fileIsDefaultLayout(file)) {
        await build(buildProps)
